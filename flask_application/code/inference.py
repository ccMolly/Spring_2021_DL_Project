...
def inference(img_name, prune_type, model_type):
    ...
    if prune_type == "filter":
        if model_type == "resnet":
            parser = build_parser()
            args = parser.parse_args(["--net", "resnet50", "--prune-flag", "--load-path", "resnet_trained_models/check_point.pth", 
        ...
        if model_type == "vgg":
            parser = build_parser()
            args = parser.parse_args(["--net", "vgg19_bn", "--prune-flag", "--load-path", "vgg_trained_models/check_point.pth",
